import os, json, time, yaml, pandas as pd, openai
...
CFG_PATH = Path(os.getenv("CONFIG_PATH") or next(p / "configs" / "config.yaml" for p in Path(__file__).resolve().parents if (p / "configs" / "config.yaml").exists()))
_cfg     = yaml.safe_load(CFG_PATH.read_text(encoding="utf-8"))

CLASSES = dget(dget(_cfg, "llm", {}), "classes", ["AE100 only", "AE200 only", "Both", "anyofthem"])
...
def finals(x, narr, final_cols): return {k: v for k, v in x.items() if any(k == f or k.endswith(f"__{f}") for f in final_cols) or k in {"original_index", narr, "skip_reason", "error"}}
...
def four_class(ae100, ae200):
    ...
    if ae100: return AE100_ONLY
    if ae200: return AE200_ONLY
    return ANY_OF_THEM
...
def call(client, msgs, llm, prm):
    ...
    for i in range(1, retries + 1):
        try:
            ...
            if model not in no_temp_models: kw["temperature"] = float(dget(prm, "temperature", 0.0))
            ...
            if isinstance(e, openai.RateLimitError) and ("insufficient_quota" in str(e) or "billing" in str(e).lower()):
...
def evaluate_labels(llm_out, gt_path, llm):
    ...
    gt = _reader(gt_path)(gt_path)[gt_cols].reset_index().rename(columns={"index": "original_index"})
    ...
    if class_col is None:
        raise ValueError(f"Missing Final_Class or Final_HFACS_Code column in LLM output. Columns: {list(llm_df.columns)}")

    _lookup = {alias: cls for cls in CLASSES for alias in (cls.lower(), cls.lower().split()[0])}
    _lookup |= {"multiple": _lookup.get("both"), "none": _lookup.get("anyofthem")}
    merged["y_true"] = merged.apply(lambda r: four_class(*[r[c] == c for c in gt_cols]), axis=1)
    merged["y_pred"] = merged[class_col].apply(lambda v: _lookup.get(str(v).strip().lower()) if not pd.isna(v) else None)
    valid = merged["y_pred"].isin(CLASSES)
    invalid = merged.loc[~valid, ["original_index", class_col]]
    y_true_v, y_pred_v = merged.loc[valid, "y_true"].tolist(), merged.loc[valid, "y_pred"].tolist()
...
def run():
    llm = dget(_cfg, "llm", {})
    data_dir = CFG_PATH.parents[1] / dget(dget(_cfg, "paths", {}), "ghfacs_data_dir", "data/GHFACS")
    if not (inp := dget(llm, "input")): raise ValueError("Missing input. Set llm.input in config.")
    prompt_name = dget(llm, "prompt", dget(llm, "default_prompt", "initial_prompting"))
    inp_path = data_dir / inp
    out = data_dir / f"{inp_path.stem}_LLM_Output_{prompt_name}{'_compact' if dget(llm, 'compact', False) else ''}.csv"
    ...
    if dget(llm, "mode") == "evaluate":
        ...
        prm = yaml.safe_load(Path(__file__).with_name(f"{prompt_name}.yaml").read_text(encoding="utf-8"))
        ...
        def process(i, n):
            ...
            final_cols = set(dget(llm, "output_columns", ["Final_Answer", "Final_Class", "Final_HFACS_Code", "Codes_Selected", "Final_Justification", "Confidence"]))
            sys_p = dget(prm, "system_prompt", dget(prm, "system", ""))
            step2_t = dget(prm, "step2_prompt")
            usr_t = dget(prm, "step1_prompt", dget(prm, "user_prompt_template", dget(prm, "prompt_template", "")))
            ...
            try:
                step1_out = call(client, msgs(usr_t.replace("{narrative}", n)), llm, prm)
                raw = call(client, msgs(step2_t.replace("{preconditions}", step1_out)), llm, prm) if step2_t else step1_out
